...
int main(int argc, char** argv) {
  ...
  if (argc > 4) {
    std::cerr << "Usage: daru_v4_mujoco_teleop_sim [mjcf.xml] [robot.urdf] [steps]\n";
    return 1;
  }
  ...
  int rc = 0;
  try {
    daru_mj::DaruV4MuJoCoController controller(model, data, urdf_path.string());
    daru_mj::DaruV4MuJoCoRosBridge ros_bridge(controller, "daru_v4_mujoco_teleop_sim");
    ...
    rc = daru_mj::RunTeleopGui(model, data, controller, ros_bridge, steps, ros_tick);
