...
struct ViewerState {
  ...
  mjData* data = nullptr;
  DaruV4MuJoCoCustomController* controller = nullptr;
  mjvCamera cam;
  mjvOption opt;
  mjvScene scn;
...
void DrawHud(const ViewerState& viewer, const DaruV4MuJoCoCustomController& controller, const mjrRect& viewport,
             int step_count);
...
void RenderSceneFrame(GLFWwindow* window,
                      ViewerState& viewer,
                      const DaruV4MuJoCoCustomController& controller,
                      int step_count,
                      bool draw_hud = true) {
  int width = 0;
  int height = 0;
...
void ShowStartupSplash(GLFWwindow* window, ViewerState& viewer, const DaruV4MuJoCoCustomController& controller) {
  UiIconTexture logo;
...
[[maybe_unused]] void BuildStatusPanel(const ViewerState& viewer, const DaruV4MuJoCoCustomController& controller,
...
void PrintTaskBoxMasses(const ViewerState& viewer) {
  const int left_body_id = viewer.task_box_body_ids[0];
  const int right_body_id = viewer.task_box_body_ids[1];
  if (left_body_id < 0 || right_body_id < 0) {
    std::cout << "[GR00T_TASK] box mass unavailable" << std::endl;
    return;
  }

  std::cout << "[GR00T_TASK] left_box_mass=" << viewer.model->body_mass[left_body_id]
            << " right_box_mass=" << viewer.model->body_mass[right_body_id] << std::endl;
}
