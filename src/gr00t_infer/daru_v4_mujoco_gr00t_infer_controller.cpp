...
const std::array<const char*, daru_mj::DaruV4MuJoCoCustomController::kDof> kJointNames = {
...
const std::array<double, daru_mj::DaruV4MuJoCoCustomController::kDof> kWorkReadyJointPose = {
...
DaruV4MuJoCoCustomController::DaruV4MuJoCoCustomController(mjModel* model, mjData* data, const std::string& urdf_path)
...
void DaruV4MuJoCoCustomController::Reset() { ResetInternal(1); }

void DaruV4MuJoCoCustomController::ResetToMode2() { ResetInternal(2); }

void DaruV4MuJoCoCustomController::ResetInternal(int start_mode) {
  world_cnt_ = 0;
  cnt_ = 0;
  mode_ = 0;
  ...
  hand_cmd_.fill(0.0);
  gr00t_ref_pos_.fill(0.0);
  gr00t_target_received_ = false;
...
void DaruV4MuJoCoCustomController::StartMode2HomeBlend() {
  mode2_home_blend_active_ = true;
  mode2_home_blend_elapsed_ = 0.0;
}

void DaruV4MuJoCoCustomController::ApplyMode2HomeBlend() {
...
void DaruV4MuJoCoCustomController::ApplyWorkReadyPose() {
  for (int i = 0; i < kDof; ++i) {
...
void DaruV4MuJoCoCustomController::SetLeftEeTarget(const RigidBodyDynamics::Math::Vector3d& pos,
...
void DaruV4MuJoCoCustomController::SetRightEeTarget(const RigidBodyDynamics::Math::Vector3d& pos,
                                               const Eigen::Quaterniond& quat) {
  const std::lock_guard<std::mutex> lock(target_mutex_);
  r_pos_des_ = pos;
...
void DaruV4MuJoCoCustomController::SetHeadTarget(const Eigen::Quaterniond& quat) 
{
  const std::lock_guard<std::mutex> lock(target_mutex_);
  head_quat_des_ = quat.normalized();
}

void DaruV4MuJoCoCustomController::SetLeftEePositionTarget(const RigidBodyDynamics::Math::Vector3d& pos) {
  const std::lock_guard<std::mutex> lock(target_mutex_);
  l_pos_des_ = pos;
}

void DaruV4MuJoCoCustomController::SetLeftHandLinearTargets(const std::array<double, 6>& targets) {
...
void DaruV4MuJoCoCustomController::SetRightHandLinearTargets(const std::array<double, 6>& targets) {
  const std::lock_guard<std::mutex> lock(hand_target_mutex_);
  for (int i = 0; i < kHandPerSide; ++i) {
...
void DaruV4MuJoCoCustomController::SetGr00tRefTargets(const std::array<double, kGr00tDof>& targets) {
  const std::lock_guard<std::mutex> lock(gr00t_target_mutex_);
  gr00t_ref_pos_ = targets;
  gr00t_target_received_ = true;
}

void DaruV4MuJoCoCustomController::InitializeHomeModeIfNeeded() {
...
void DaruV4MuJoCoCustomController::Step() {
...
void DaruV4MuJoCoCustomController::ApplyGr00tTargets() {
  std::array<double, kGr00tDof> targets{};
  {
    const std::lock_guard<std::mutex> lock(gr00t_target_mutex_);
    if (!gr00t_target_received_) {
      return;
    }
    targets = gr00t_ref_pos_;
  }

  des_pos_[0] = targets[28];
  for (int i = 0; i < 7; ++i) {
    des_pos_[1 + i] = targets[i];
    des_pos_[8 + i] = targets[7 + i];
  }

  for (int i = 0; i < kDof; ++i) {
    ref_pos_(i) = des_pos_[i];
    joints_[i].R_pos = des_pos_[i];
  }

  if (head_map_joint_[0] >= 0) {
    head_des_pos_[0] = targets[26];
  }
  if (head_map_joint_[1] >= 0) {
    head_des_pos_[1] = targets[27];
  }

  const std::lock_guard<std::mutex> lock(hand_target_mutex_);
  for (int i = 0; i < kHandPerSide; ++i) {
    const int right_idx = i;
    if (hand_map_joint_[right_idx] >= 0) {
      hand_des_pos_[right_idx] = targets[20 + i];
    }

    const int left_idx = kHandPerSide + i;
    if (hand_map_joint_[left_idx] >= 0) {
      hand_des_pos_[left_idx] = targets[14 + i];
    }
  }
}

void DaruV4MuJoCoCustomController::StateUpdate() {
  for (int i = 0; i < kDof; ++i) {
    const int qadr = map_qpos_[i];
    const int dadr = map_dof_[i];
...
void DaruV4MuJoCoCustomController::ApplyArmTorque() {
  mju_zero(d_->qfrc_applied, m_->nv);
  for (int i = 0; i < kDof; ++i) {
...
void DaruV4MuJoCoCustomController::GetCurrentState() {
...
std::array<double, 12> DaruV4MuJoCoCustomController::GetFingerForceState()
{
...
void DaruV4MuJoCoCustomController::SetZeroPos() {
...
void DaruV4MuJoCoCustomController::RbdlStateUpdate() {
  for (int i = 0; i < kDof; ++i) {
...
void DaruV4MuJoCoCustomController::IKSolver(RigidBodyDynamics::Math::Vector3d l_pos_des,
...
void DaruV4MuJoCoCustomController::RefTorqueInit() { std::memset(des_torque_, 0, sizeof(double) * kDof); }

void DaruV4MuJoCoCustomController::CGCompensation(double k) {
...
std::array<double, DaruV4MuJoCoCustomController::kDof> DaruV4MuJoCoCustomController::JointPosition() const {
...
std::array<double, DaruV4MuJoCoCustomController::kDof> DaruV4MuJoCoCustomController::JointRefPosition() const {
  std::array<double, kDof> out{};
  for (int i = 0; i < kDof; ++i) {
...
std::array<double, DaruV4MuJoCoCustomController::kDof> DaruV4MuJoCoCustomController::JointVelocity() const {
  std::array<double, kDof> out{};
  for (int i = 0; i < kDof; ++i) {
...
std::array<double, DaruV4MuJoCoCustomController::kDof> DaruV4MuJoCoCustomController::JointAcceleration() const {
  std::array<double, kDof> out{};
  for (int i = 0; i < kDof; ++i) {
...
std::array<double, DaruV4MuJoCoCustomController::kDof> DaruV4MuJoCoCustomController::JointEffort() const {
  std::array<double, kDof> out{};
  for (int i = 0; i < kDof; ++i) {
...
std::array<double, 2> DaruV4MuJoCoCustomController::HeadPosition() const {
...
std::array<double, 2> DaruV4MuJoCoCustomController::HeadRefPosition() const {
  std::array<double, 2> out{};
  for (int i = 0; i < kHeadDof; ++i) {
...
std::array<double, 2> DaruV4MuJoCoCustomController::HeadVelocity() const {
  std::array<double, 2> out{};
  for (int i = 0; i < kHeadDof; ++i) {
    out[i] = head_vel_[i];
  }
...
std::array<double, 2> DaruV4MuJoCoCustomController::HeadEffort() const {
  std::array<double, 2> out{};
  for (int i = 0; i < kHeadDof; ++i) {
    out[i] = head_tau_[i];
  }
...
std::array<double, 12> DaruV4MuJoCoCustomController::HandPosition() const {
  std::array<double, 12> out{};
  for (int i = 0; i < kHandDof; ++i) {
...
std::array<double, 12> DaruV4MuJoCoCustomController::HandRefPosition() const {
  std::array<double, 12> out{};
  for (int i = 0; i < kHandDof; ++i) {
...
std::array<double, 12> DaruV4MuJoCoCustomController::HandVelocity() const {
  std::array<double, 12> out{};
  for (int i = 0; i < kHandDof; ++i) {
...
std::array<double, 12> DaruV4MuJoCoCustomController::HandEffort() const {
  std::array<double, 12> out{};
  for (int i = 0; i < kHandDof; ++i) {
...
std::array<double, 14> DaruV4MuJoCoCustomController::RefEePose() const {
...
std::array<double, 14> DaruV4MuJoCoCustomController::ActEePose() const {
  std::array<double, 14> out{};
...
std::array<double, 14> DaruV4MuJoCoCustomController::RealEePose() {
  std::array<double, 14> out{};
...
void DaruV4MuJoCoCustomController::JointPdControl(bool cur_reset) {
  for (int n = 0; n < kDof; ++n) {
...
void DaruV4MuJoCoCustomController::JointTorqueControl(bool cur_reset) {
  for (int n = 0; n < kDof; ++n) {
    if (cur_reset) {
...
void DaruV4MuJoCoCustomController::HandControlTick() {
...
void DaruV4MuJoCoCustomController::HeadPdControl() {
